...
void VisualWidget::update(const rbd::parsers::Visual & visual, const sva::PTransformd & pose)
{
  ...
  }
  {
    ...
    auto & visual_data = visual.geometry.data;
    if(type == Type::BOX)
    {
      auto & b = boost::get<Geometry::Box>(visual_data);
    ...
    {
      auto & c = boost::get<Geometry::Cylinder>(visual_data);
    ...
    {
      auto & s = boost::get<Geometry::Sphere>(visual_data);
    ...
    {
      auto & m = boost::get<Geometry::Mesh>(visual_data);
